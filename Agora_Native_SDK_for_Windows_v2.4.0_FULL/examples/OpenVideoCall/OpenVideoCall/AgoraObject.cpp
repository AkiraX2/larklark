...
BOOL CAgoraObject::JoinChannel(LPCTSTR lpChannelName, UINT nUID)
{
	...
	// 设置双声道高音质
	int nRet = 0;
	//m_lpAgoraEngine->setAudioProfile(AUDIO_PROFILE_MUSIC_HIGH_QUALITY_STEREO, AUDIO_SCENARIO_DEFAULT);
	m_lpAgoraEngine->setAudioProfile(AUDIO_PROFILE_MUSIC_HIGH_QUALITY_STEREO, AUDIO_SCENARIO_SHOWROOM);

	//	rep->setParameters("{\"che.audio.enable.agcOn\":false}");
	// m_lpAgoraEngine->setParameters("{\"che.audio.enable.agcOn\":false}");

	AParameter apm(*m_lpAgoraEngine);
	//apm->setParameters( "{\"che.audio.enable.agcOn\":false}");
	apm->setParameters( "{\"che.audio.enable.agc\":false}");
	apm->setParameters( "{\"che.audio.enable.aec\":false}"); // 回音
	apm->setParameters( "{\"che.audio.enable.ns\":false}" );
	apm->setParameters( "{\"che.audio.stereo.capture\": true}" );

	rep.enableAudioVolumeIndication(200,  3);
