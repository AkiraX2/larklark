...
BEGIN_MESSAGE_MAP(CVideoDlg, CDialogEx)
	...
	ON_MESSAGE(WM_MSGID(EID_USER_MUTE_AUDIO), &CVideoDlg::OnRemoteMuteChanged)
	ON_MESSAGE(WM_MSGID(EID_AUDIO_VOLUME_INDICATION), &CVideoDlg::OnRemoteAudioVolumeIndication)
...
const int WIDTH = 192*0.6;
const int HEIGHT = 144*0.6;
...
void CVideoDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	m_rcChildVideoArea.bottom = m_rcChildVideoArea.top + HEIGHT;
	m_rcChildVideoArea.right = m_rcVideoArea.right - 14;
	m_rcChildVideoArea.left = m_rcChildVideoArea.right - WIDTH;
...
void CVideoDlg::AdjustSizeVideoMulti(int cx, int cy)
{
	...
	for (int nIndex = 0; nIndex < 16; nIndex++) {
		int nXPos = (m_rcVideoArea.Width() / 2) - 402 + ((WIDTH+16) * nLocalIndex);
		int nYPos = m_rcVideoArea.top + 16;

		if (!m_wndVideo[nIndex].IsBigShow()) {
			if (m_wndVideo[nIndex].GetUID() != 0) {
				m_wndVideo[nIndex].MoveWindow(nXPos, nYPos, WIDTH, HEIGHT, TRUE);
				nLocalIndex++;
			}
		}
		else{
			m_wndLocal.MoveWindow(nXPos, nYPos, WIDTH, HEIGHT, TRUE);
			nLocalIndex++;
		}
	}
}
...
LRESULT CVideoDlg::OnRemoteVideoStat(WPARAM wParam, LPARAM lParam)
{
	...
	while (posNext != NULL) {
		...
		if (rWndInfo.nUID == lpData->uid) {
			...
			//m_wndVideo[rWndInfo.nIndex].SetMute((rWndInfo.bMute);
			break;
		}
	}
...
LRESULT CVideoDlg::OnRemoteMuteChanged(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CVideoDlg::OnRemoteAudioVolumeIndication(WPARAM wParam, LPARAM lParam)
{
	...
	while (posNext != NULL) {
		...
		for (int i=0; i < lpData->speakerNumber;i++)
		{
			if (rWndInfo.nUID == lpData->speakers[i].uid) {

				m_wndVideo[rWndInfo.nIndex].SetSpeakerAudioVolume(lpData->speakers[i].volume, lpData->totalVolume);
				//m_wndVideo[rWndInfo.nIndex].SetMute((rWndInfo.bMute);
				break;
			}
		}
...
void CVideoDlg::ShowVideo4()
{
	...
	int Width = m_rcVideoArea.Width() / 4;
	int Height = m_rcVideoArea.Height() / 4;

	m_wndVideo[0].MoveWindow(0,			m_rcVideoArea.top,		Width,		Height,		 FALSE);
	m_wndVideo[1].MoveWindow(Width,		m_rcVideoArea.top,		Width, Height, FALSE);
	m_wndVideo[2].MoveWindow(Width*2,	m_rcVideoArea.top,		Width, Height, FALSE);
	m_wndVideo[3].MoveWindow(Width*3,	m_rcVideoArea.top,		Width, Height, FALSE);
 
	m_wndVideo[4].MoveWindow(0, m_rcVideoArea.top+Height, Width, Height, FALSE);
	m_wndVideo[5].MoveWindow(Width, m_rcVideoArea.top+Height, Width, Height, FALSE);
	m_wndVideo[6].MoveWindow(Width * 2, m_rcVideoArea.top+Height, Width, Height, FALSE);
	m_wndVideo[7].MoveWindow(Width * 3, m_rcVideoArea.top+Height, Width, Height, FALSE);

	m_wndVideo[8].MoveWindow(0, m_rcVideoArea.top + 2 * Height, Width, Height, FALSE);
	m_wndVideo[9].MoveWindow(Width, m_rcVideoArea.top + 2*Height, Width, Height, FALSE);
	m_wndVideo[10].MoveWindow(Width * 2, m_rcVideoArea.top + 2 * Height, Width, Height, FALSE);
	m_wndVideo[11].MoveWindow(Width * 3, m_rcVideoArea.top + 2 * Height, Width, Height, FALSE);

	m_wndVideo[12].MoveWindow(0, m_rcVideoArea.top + 3*Height, Width, Height, FALSE);
	m_wndVideo[13].MoveWindow(Width, m_rcVideoArea.top + 3 * Height, Width, Height, FALSE);
	m_wndVideo[14].MoveWindow(Width * 2, m_rcVideoArea.top + 3 * Height, Width, Height, FALSE);
	m_wndVideo[15].MoveWindow(Width * 3, m_rcVideoArea.top + 3 * Height, Width, Height, FALSE);
	...
	if (nRemoteCount == 2)
		m_wndLocal.MoveWindow(0, m_rcVideoArea.top + m_rcVideoArea.Height() / 4, m_rcVideoArea.Width() / 4, m_rcVideoArea.Height() / 4, FALSE);
	else
		m_wndLocal.MoveWindow(Width*3, m_rcVideoArea.top + Height*3, Width, Height , FALSE);
...
void CVideoDlg::RebindVideoWnd()
{
	...
	if (m_nScreenMode != SCREEN_VIDEOMULTI) {
		if (m_listWndInfo.GetCount() <= 1)
			ShowVideo1();
			// 4 -> 16
		else if (m_listWndInfo.GetCount() > 1 && m_listWndInfo.GetCount() < 16)
			ShowVideo4();
		else
			ShowMulti();
	}
